...
function init() {
	...
	function eachSettingAs(nameType) {
		...
		function mapKeys(accessor, keys) {
			_.forIn(keys, function each(value, key) {
				if (isSimple(value)) {
					const newValue = accessor(nameFromKey(key, nameType));
	...
	function enableAndDisableFeatures(accessor, nameType) {
		...
		function getAndPrepare(key) {
			const raw = accessor(nameFromKey(key, nameType)) || '';
			const cleaned = decodeURIComponent(raw).toLowerCase();
		...
		function prepareAlarmTypes() {
			let alarmTypes = _.filter(getAndPrepare('alarmTypes'), function onlyKnownTypes(type) {
				return type === 'predict' || type === 'simple';
			});

			if (alarmTypes.length === 0) {
				const thresholdWasSet = _.findIndex(wasSet, function (name) {
		...
		const enable = getAndPrepare('enable');
		const disable = getAndPrepare('disable');
	...
	function adjustShownPlugins() {
		const showPluginsUnset = settings.showPlugins && 0 === settings.showPlugins.length;
